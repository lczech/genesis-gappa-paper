...
testnames = [ "edgepca", "squash", "kr", "edpl" ]
...
def plot_test(testname, data, measure):
	...
	if testname == "edgepca":
		title = "Edge PCA"
	elif testname == "squash":
		title = "Squash Clustering"
	elif testname == "kr":
		title = "KR Distance"
	elif testname == "edpl":
		title = "EDPL"
	...
	for index, row in data.loc[data[0].str.contains(testname)].iterrows():
		...
		# Make a special style for gappa: solid line at the front of the list,
		# because we are going to move the gappa column to the front as well.
	...
	# Move gappa column to front. It's the one we are interested in here.
	cols = list(sub_data)
	cols.insert(0, cols.pop(cols.index('gappa')))
	...
	if testname == "edpl":
		...
		size_names = ['10', '20', '50', '100', '200', '500', '1K', '2K', '5K', '10K']
